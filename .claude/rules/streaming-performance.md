...
- **未交付边界 API（UI52–66）**：`GET /admin/security-policies` 的 `remainders` 恒为 `backend_sql_with_hold=false` / `crdt_merge=false` / `mlock=false` / `process_rss_window_byte_ci=false`；全 `smoke-security-*.sh` + units `ui52_…` + `ui57_security_off_still_exposes_remainders_honesty` 钉住。相邻 `sql_cursor` / `peak_is_process_rss` 诚实字段**不**表示 A10/H05/A06 重债已完成。详见架构文档 **§13.3**。
