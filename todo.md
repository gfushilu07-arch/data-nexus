...
1. **A10** backend SQL `DECLARE … WITH HOLD` 服务端游标（可选；进程内 + remainders 诚实 UI52–60 + unit on/off + L0/auth smoke pin 已有）  
