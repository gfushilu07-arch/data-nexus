...
### 关键 smoke（矩阵规模；发版前 `all`+`cedar`；**UI52–71 remainders 诚实 pin 已覆盖 security + L0 dual/xproto + admin-auth**）
...
| SQLT-4B1 | 双向跨协议 DQL spec/oracle、真实 MySQL text 与 PostgreSQL simple client、backend control、隔离 Docker runner、24/24 path 与零残留审计 | `873522d`、`a1d9065`、`497cf68` |
| SQLT-4B2 | 双向跨协议 DML/事务 spec/oracle、同连接 wire transcript、精确 affected/error/transaction/final-state 证据、16/16 path 与零残留审计 | `3903751`、`62b87a6` |
...
- [x] **H01–H04**（含 H04b 本地 Docker + 浏览器代码侧验收）
- [x] **UI01 / UI02**  
- [x] **A05** 透传路径观测  
- [x] **S0–S6 / A1–A4 / v1 L0**  
...
| UI62 | bump remainders honesty spans UI52–71 + A09 仍欠 RSS cite | docs(ui62) |
...
| UI67 | bump remainders honesty spans to UI52–71 after helper matrix | docs(ui67) |
...
| SQLT-4B3 | translation prepared open-up + token-aware ?↔$n rewrite (0126a90) | feat(SQLT-4B3) |
