...
| 进程内 secret | 活跃 lease 的 backend 密码在 RAM；**revoke / prune / store Drop** 时 **ZeroizeOnDrop** 擦除；Admin `security-policies.state` 暴露 **`vault_password_zeroize=true` / `mlock=false`**（**非** mlock / 安全堆）；`backend_identity` 返回 `(user, Zeroizing<String>)`；顶层 **`remainders.mlock=false`**（UI52–57）再次钉住 |
...
- 多实例文件态非 CRDT（last-writer-wins；API `merge_strategy` / `crdt=false`；`remainders.crdt_merge=false`）。  
- 进程内 vault 密码：活跃期在 RAM；revoke/prune/Drop 时 Zeroize（API `vault_password_zeroize=true`，**非** mlock；`remainders.mlock=false`）；`backend_identity` 返回 `Zeroizing` 副本。  
- Portal/会话 SQL 游标是进程内命名游标，**非** backend `DECLARE … WITH HOLD`（`remainders.backend_sql_with_hold=false`）。  
- 编码峰值是逻辑窗，**非**精确 1–2 窗 process RSS CI（`remainders.process_rss_window_byte_ci=false`）。  
