...
| 进程内 secret | 活跃 lease 的 backend 密码在 RAM；**revoke / prune / store Drop** 时 **ZeroizeOnDrop** 擦除；Admin `security-policies.state` 暴露 **`vault_password_zeroize=true` / `mlock=false`**（**非** mlock / 安全堆）；`backend_identity` 返回 `(user, Zeroizing<String>)`；顶层 **`remainders.mlock=false`**（UI52–71）再次钉住 |
