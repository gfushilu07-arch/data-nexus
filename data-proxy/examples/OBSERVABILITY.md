...
| A10/H05/A06 heavy remainders “done” | Admin always returns **`remainders.backend_sql_with_hold=false`**, **`crdt_merge=false`**, **`mlock=false`**, **`process_rss_window_byte_ci=false`** (UI52–61). Process-local SQL cursors, LWW file state, logical encode peak remain the real guarantees. |
...
| Stale-span cleanup (UI58) | todo/rules/OBSERVABILITY cite UI52–61 + A-track remainders claim | Keep honesty span numbers current |
...
| Span hygiene (UI62) | todo/rules/runbook/matrix cite UI52–61 consistently | Avoid stale honesty-span numbers |
