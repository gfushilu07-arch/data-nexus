...
| Stale-span cleanup (UI58) | todo/rules/OBSERVABILITY cite UI52–71 + A-track remainders claim | Keep honesty span numbers current |
...
| Span hygiene (UI62) | todo/rules/runbook/matrix cite UI52–71 consistently | Avoid stale honesty-span numbers |
...
| Span hygiene (UI67) | todo/rules/runbook/matrix cite UI52–71 after full helper wiring | Keep honesty spans current |
...
| Helper path hygiene (UI70) | dual-listener uses dirname helper path; spans UI52–71 | Consistent helper invocation across L0 smokes |
| Helper coverage gate (UI71) | `check-honesty-helper-coverage.sh` (26 smokes) + §5 honesty closed | Prevent silent drop of helper pins |
