...
| A10/H05/A06 heavy remainders “done” | Admin always returns **`remainders.backend_sql_with_hold=false`**, **`crdt_merge=false`**, **`mlock=false`**, **`process_rss_window_byte_ci=false`** (UI52–57). Process-local SQL cursors, LWW file state, logical encode peak remain the real guarantees. |
