...
| A10/H05/A06 heavy remainders “done” | Admin always returns **`remainders.backend_sql_with_hold=false`**, **`crdt_merge=false`**, **`mlock=false`**, **`process_rss_window_byte_ci=false`** (UI52–69). Process-local SQL cursors, LWW file state, logical encode peak remain the real guarantees. |
...
| Stale-span cleanup (UI58) | todo/rules/OBSERVABILITY cite UI52–69 + A-track remainders claim | Keep honesty span numbers current |
...
| Span hygiene (UI62) | todo/rules/runbook/matrix cite UI52–69 consistently | Avoid stale honesty-span numbers |
...
| Span hygiene (UI67) | todo/rules/runbook/matrix cite UI52–69 after full helper wiring | Keep honesty spans current |
...
| Helper path hygiene (UI70) | dual-listener uses dirname helper path; spans UI52–69 | Consistent helper invocation across L0 smokes |
...
| Portal HTTP card (UI41) | `gateway_execute_path_total{type=PORTAL_STREAM\|PORTAL_CHUNKED}` + stream peak rows | `PORTAL_STREAM` ≈ `backend_window`; `PORTAL_CHUNKED` may materialize backend ResultSet; peak still **logical** |
| Secure path card (UI42) | `gateway_mask_rows_total` + `gateway_passthrough_bytes_total` | Mask rows = non-empty mask obligation only; passthrough bytes = wire payload — **not** RSS proof |
