...
echo "==> UI60 GET /admin/security-policies without token → 401"
code="$(curl -s -o /tmp/dn-ui60-policies-401.json -w "%{http_code}" "$ADMIN/admin/security-policies")"
[[ "$code" == "401" ]] || {
  echo "expected security-policies 401 without token, got $code"
  exit 1
}
