...
done < <(python3 "$ROOT/select_cursor_cases.py" "$ROOT/manifest.json" "$ROOT/cursor-oracles.json")

python3 - "$RESULTS" "$RUN_DIR/summary.json" "$case_count" "$path_count" "$pass_count" "$fail_count" <<'PY'
import json, sys
from pathlib import Path
...
if summary["case_dialect_executions"] != 8 or summary["path_executions"] != 18:
